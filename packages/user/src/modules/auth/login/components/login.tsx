...
function Login({ submit, loading, messageError, setMessageError }: Props) {
  ...
  return (
    <Container className="min-h-full flex items-center justify-center">
      <div className="bg-white px-48 pt-24 pb-[11rem] flex flex-col rounded-md border border-solid border-[#CCCCCC]">
        <Form className="w-[28.25rem] flex flex-col" onFinish={form.handleSubmit} layout="vertical">
          ...
          <div className="h-[1px] bg-gray mt-14 mb-12" />
          <p className="text-lg self-center text-black-lighter-2 mb-[1.375rem]">未登録の企業様はこちら</p>
          <Link href="/register">
            <a className="h-[3.375rem] border-solid border-[1px] rounded-sm border-black-lighter-3 bg-[#F1F1F1] text-black-lighter-2 flex items-center font-bold justify-center text-lg hover:border-primary hover:text-primary hover:bg-[#FFEFF0]">
              アカウント登録申請
            </a>
          </Link>
        </Form>
